...
{
	 mysqli_query("DELETE FROM examination WHERE subid = '$subid[$j]'");
...
for($i=0;$i<$_POST["tst"];$i++)
{
	for($j=0;$j<$_POST['tsubb'];$j++)
	{
mysqli_query("UPDATE examination SET scored='$submark[$j]',percentage ='$resul[$j]', result='$comment[$j]' WHERE examid ='$_POST[examid]' AND subid='$subid[$j]' AND studid='$studid[$i]'");
...
$rescourse = mysqli_query("SELECT * FROM course where courseid='$_POST[course]'");
while($row1 = mysqli_fetch_array($rescourse))
  {
	$courseid =   $row1["courseid"];
	$coursename =   $row1["coursename"];
  }
  $resclass = mysqli_query("SELECT * FROM subject where subid='$_POST[subject]'");
while($row2 = mysqli_fetch_array($resclass))
  ...
  */
    ...
    $restotst = mysqli_query("SELECT * FROM studentdetails 
 where courseid='$_POST[course]' AND semester ='$_POST[semester]'");
$tstudent = mysqli_num_rows($restotst);

  $restotstrec = mysqli_query("SELECT * FROM studentdetails where courseid='$_POST[course]' AND semester='$_POST[semester]'");

$ressub = mysqli_query("SELECT * FROM subject where courseid='$_POST[course]' AND semester='$_POST[semester]'");

$restotsub = mysqli_query("SELECT * FROM subject where courseid='$_POST[course]' AND semester='$_POST[semester]'");
...
<input type="hidden" value="<?php echo $tstudent; ?>" name="tst"/>
       <?php
	   $ressubb = mysqli_query("SELECT * FROM subject where courseid='$_POST[course]' AND semester='$_POST[semester]'");
